...
def filter_findings(findings, field, value):
    """Return findings where field == value."""
    return [row for row in findings if row.get(field) == value]


def top_subdomains(findings, n):
    """Return top n subdomains by number of findings."""
